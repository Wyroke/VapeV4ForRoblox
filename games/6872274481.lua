...
run(function()
	...
	local function getAttackData()
		...
		if LegitAura.Enabled or ClosetMode.Enabled then
			if (tick() - bedwars.SwordController.lastSwing) >= 0 then
				CanHit = false
				return false 
			else
				CanHit = true
						...
						if #plrs > 0 then
							...
							for _, v in plrs do
								...
								if actualRoot then
									...
									bedwars.SwordController.lastSwingServerTime = SyncHit.Enabled and workspace:GetServerTimeNow() - (tick() * 2e8) or workspace:GetServerTimeNow() - tick()
...
SwingRange = Killaura:CreateSlider({
	...
	MutiAura = Killaura:CreateToggle({
		...
		Default = false,
		Function = function(v)
			MutiAuraDelay.Object.Visible = (not v)
		end
	})
	...
	BetterFisher = vape.Categories.Kits:CreateModule({
		...
		Function = function(callback)
			if store.equippedKit ~= "fisherman" then
				vape:CreateNotification("BetterFisher", "Kit required only spec was here!", 6, "warning")
				return
			end
			if callback then
